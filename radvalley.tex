...
stars (see Fig.~\ref{fig:fmapF}) has implications for where in the planetary parameter space one expects to find
predominantly terrestrial or gaseous planets. Fig.~\ref{fig:transition} reveals the region of interest for resolving
...
$M_s$-dependent scaling relation of the radius valley under photoevaporation \citep{wu19}. The measured transitions
intersect at $P\sim 23.52$ days such that two regions of interest emerge and are bounded by}

\begin{equation}
  r_p \in 
  \begin{cases}
        [r_{p,\text{this work}}, r_{p,\text{M19}}], & \quad P < 23.52 \text{ days,} \\
        [r_{p,\text{M19}}, r_{p,\text{this work}}], & \quad P > 23.52 \text{ days.} \\
...
\begin{align}
  r_{p,\text{this work}} &= 0.11\cdot \log_{10}(P)+1.52, \\
  r_{p,\text{M19}} &= -0.48\cdot \log_{10}(P)+2.32.
\end{align}

\noindent \textbf{These subsets of the $P-r_p$ parameter space define the regions of interest for resolving the
  terrestrial-to-gaseous transition around low mass stars. For example, at $P\lesssim 23.52$ days, thermally driven
  atmospheric mass loss, such as that from photoevaporation, predicts that planets in the set defined in
  Eq.~\ref{eq:transition} should be predominantly rocky. Whereas the gas-poor formation scenario, whose predicted
  radius valley slope differs in sign from that of thermally driven mass loss, would predict that those planets be
  ...
  As seen in Fig.~\ref{fig:transition}, there are only $\sim 5$ planets in the region of interest with $\geq 3\sigma$ bulk
  density measurements. These planets highlight the decrease in bulk density with increasing $r_p$ although insufficient
  information
  is available to resolve a possibly sharp transition. The \tess{} SPOC has already identified three TOIs of interest
  around stars with \teff{} $<4700$ K\footnote{TOIs: 654.01, 732.02, and 1184.01.} that should be targeted by RV follow-up
  campaigns.}
...
\begin{figure}
  ...
  \caption{Regions of interest for resolving the transition between terrestrial and gaseous planets
    ...
    Fig.~\ref{fig:fmap}. The terrestrial-to-gaseous transition measured in this work is highlighted
    ...
    these curves represent the most interesting subset of the $P-r_p$ parameter space for resolving
    the transition around low mass stars from precise bulk density measurements. TOIs from the
    SPOC that fall within the region of interest are marked by an `X'. Also overplotted are planets
    with $\geq 3\sigma$ bulk density measurements from the literature that are classified as having either
    a terrestrial (\emph{circles}), a gaseous (\emph{triangles}), or an ambiguous (\emph{squares}) bulk
    composition.}
