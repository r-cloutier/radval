...
\author{Kristen Menou\altaffilmark{3,2,4}}
...
\altaffiltext{4}{Dept. of Physics,
...
\begin{abstract}
  ...
  %while exploiting the \gaia{} DR2 data to refine the stellar parameters. 
  Applying completeness corrections %to the empirical planet population
...
\citep[e.g.][]{fulton17,berger18,fulton18,vaneylen18,martinez19,hardegree20}.
Each of these independent studies clearly
...
\begin{figure*}
  \centering
  \includegraphics[width=\hsize]{figures/Ndetmap_map_Msgt0d0_xbin45_ybin27.png}%
  \hspace{-\hsize}%
  \begin{ocg}{fig:offNd}{fig:offNd}{0}%
  \end{ocg}%
  \begin{ocg}{fig:onNd}{fig:onNd}{1}%
  \includegraphics[width=\hsize]{figures/Ndetmap_kepk2_Msgt0d0_xbin45_ybin27.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \caption{(\emph{Interactive figure}) Empirical population of confirmed close-in planets around low mass stars.
    The distribution of 275 and 53 confirmed planets from \kepler{}
    and \ktwo{}
    \ToggleLayer{fig:onNd,fig:offNd}{\protect\cdbox{(\emph{circles and diamonds} respectively)}}
    as a function of orbital period, insolation, and planet radius. The two-dimensional maps are
...
The empirical planet population in \autoref{fig:Ndet} exhibits many recognizable features in the distribution of
...
significant subset of confirmed planets between 1.5-1.8 R$_{\oplus}$ with periods out to $\sim 12$ days indicating that
the mechanism responsible for producing the radius valley might not be as efficient as when operating on planetary
...
\begin{figure*}
  \centering
  \includegraphics[width=.9\hsize]{figures/fmap_bkgd_Msgt0d0_xbin39_ybin26.png}%
  \hspace{-.9\hsize}%
  \begin{ocg}{fig:off3}{fig:off3}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on3}{fig:on3}{1}%
  \includegraphics[width=.9\hsize]{figures/fmap_kepk2_Msgt0d0_xbin39_ybin26.png}%
  \end{ocg}
  \hspace{-.9\hsize}%
  \begin{ocg}{fig:off1}{fig:off1}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on1}{fig:on1}{1}%
  \includegraphics[width=.9\hsize]{figures/fmap_curve_Msgt0d0_xbin39_ybin26.png}%
  \end{ocg}
  \hspace{-.9\hsize}%
  \begin{ocg}{fig:off4}{fig:off4}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on4}{fig:on4}{1}%
  \includegraphics[width=.9\hsize]{figures/fmap_planets_Msgt0d0_xbin39_ybin26.png}%
  \end{ocg}
  \hspace{-.9\hsize}%
  \caption{(\emph{Interactive figure}) Planet occurrence rate versus orbital period and planetary radius. The maximum a-posteriori occurrence
    rate map calculated from the population of confirmed planets from \kepler{} and
    \ktwo{} around low mass dwarf stars \ToggleLayer{fig:on3,fig:off3}{\protect\cdbox{(\emph{small circles and diamonds})}}.
    For comparison, overplotted are the \ToggleLayer{fig:on1,fig:off1}{\protect\cdbox{empirical locations}}
    of the radius valley around FGK stars 
    characterized via asteroseismology \citep[\emph{dashed line},][]{vaneylen18} and the approximate radius
    valley around mid-K to early-M dwarfs \citep[\emph{dotted line},][]{wu19}. Also overplotted are
    \ToggleLayer{fig:on4,fig:off4}{\protect\cdbox{planets}}
    with $\geq 3\sigma$ bulk density measurements from the literature that are classified as having either
    a rocky (\emph{circles}), a gaseous (\emph{triangles}), or an intermediate (\emph{squares}) bulk
...
\textbf{To test the statistical significance of the bimodality in \autoref{fig:fmap}, we used
  ...
  $\Delta \text{BIC}_{34}=BIC_3-BIC_4\sim -10$ such that it is strongly favored over two components but only
...
\textbf{The uncertainty on each $f_j$ bin is computed from the $16^{\text{th}}$ and $84^{\text{th}}$ percentiles
  of its $f_j$ posterior and are thus correlated because the two-dimensional occurrence rates
  (\autoref{fig:fmap}) are smoothly varying such that adjacent radius bins are not independent.}
...
bins. For each one-dimensional distribution,
the $f_j$-weighted radii of the rocky and non-rocky peaks are then calculated.
...
\begin{figure*}
  \centering
  \includegraphics[width=0.85\hsize]{figures/fmapFmart_bkgd_Msgt0d0_xbin39_ybin26_insolation.png}%
  \hspace{-0.85\hsize}%
  \begin{ocg}{fig:off1F}{fig:off1F}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on1F}{fig:on1F}{1}%
  \includegraphics[width=0.85\hsize]{figures/fmapFmart_kepk2_Msgt0d0_xbin39_ybin26_insolation.png}%
  \end{ocg}
  \hspace{-0.85\hsize}%
  \begin{ocg}{fig:off2F}{fig:off2F}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on2F}{fig:on2F}{1}%
  \includegraphics[width=0.85\hsize]{figures/fmapFmart_slope_Msgt0d0_xbin39_ybin26_insolation.png}%
  \end{ocg}
  \hspace{-0.85\hsize}%
  \begin{ocg}{fig:off3F}{fig:off3F}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on3F}{fig:on3F}{1}%
  \includegraphics[width=0.85\hsize]{figures/fmapFmart_models_Msgt0d0_xbin39_ybin26_insolation.png}%
  \end{ocg}
  \hspace{-0.85\hsize}%
  \begin{ocg}{fig:off4F}{fig:off4F}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on4F}{fig:on4F}{1}%
  \includegraphics[width=0.85\hsize]{figures/fmapFmart_planets_Msgt0d0_xbin39_ybin26_insolation.png}%
  \end{ocg}
  \hspace{-0.85\hsize}%
  \caption{(\emph{Interactive figure})
    Planet occurrence rates versus insolation and planet radius around low mass and Sun-like stars.
    \emph{Upper panel}: the maximum a-posteriori occurrence
    rate map calculated from the population of confirmed planets from \kepler{} and
    \ktwo{} around low mass dwarf stars
    \ToggleLayer{fig:on1F,fig:off1F}{\protect\cdbox{(\emph{small circles and diamonds})}}.
    Overplotted in black are
    \ToggleLayer{fig:on3F,fig:off3F}{\protect\cdbox{model predictions}} of the transition from rocky to non-rocky
    planets in the following scenarios: core-powered mass loss \citep{gupta19b}, photoevaporation
    \citep{lopez18}, and gas-poor formation \cite{lopez18}. We measure the
    \ToggleLayer{fig:on2F,fig:off2F}{\protect\cdbox{slope of the radius valley}}
    to be $r_{p,\text{valley}} \propto F^{-0.060\pm 0.025}$ which is consistent with predictions
    from gas-poor formation of terrestrial planets. Also overplotted are
    \ToggleLayer{fig:on4F,fig:off4F}{\protect\cdbox{planets}}
    with $\geq 3\sigma$ bulk density measurements from the literature that are classified as having either
    a rocky (\emph{circles}), a gaseous (\emph{triangles}), or an intermediate (\emph{squares}) bulk
    composition. Marker colors are indicative of the MAP planet bulk densities.
    \emph{Lower panel}: the occurrence rate map of close-in
    \kepler{} planets around Sun-like stars \citep[$r_{p,\text{valley}} \propto F^{0.12\pm 0.02}$;][]{martinez19}.
    Note the unique $F$ and $r_p$ scales depicted in each panel.}
  \label{fig:fmapF}
\end{figure*}

The negative slope of $r_{p,\text{valley}} \propto F^{-0.06}$ indicates that the location of the radius valley
drops to smaller planet radii with increasing insolation (i.e. towards smaller orbital separations).
This behavior is broadly consistent with models of the formation small rocky planets in a gas-poor environment
...
$\sigma_{\text{solid}} \propto a^{-1.6}$ where $a$ is the orbital distance. The amount of solid material 
accreted by a proto-planet
is proportional to its Hill radius $r_H = a(m_p/3M_s)^{1/3}$ such that integrating over the feeding zone within
the disk surface results
...
\citep[$m_p \propto r_p^{3.7}$;][]{zeng16}.
Hence, the transition radius is predicted to occur at larger planet radii with increasing
separation for a given host spectral type \citep{lopez18}. The corresponding theoretical scaling of the transition
radius with insolation for a given spectral type is $r_{p,\text{valley}} \propto F^{-0.08}$
which is consistent with our measured scaling of $r_{p,\text{valley}}\propto F^{-0.060\pm 0.025}$.

In addition to being consistent with predictions from gas-poor terrestrial planet formation models,
the measured slope is inconsistent with models of thermally driven atmospheric
escape from photoevaporation or core-powered mass loss. These models predict
...
\begin{figure*}
  \centering
  \includegraphics[width=.98\hsize]{figures/rphist_bkgd_compMs.png}%
  \hspace{-.98\hsize}%
  \begin{ocg}{fig:off4r}{fig:off4r}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on4r}{fig:on4r}{1}%
  \includegraphics[width=.98\hsize]{figures/rphist_kepk2_compMs.png}%
  \end{ocg}
  \hspace{-.98\hsize}%
...
\textbf{This selection is purely empirical and disregards any physical motivation for a maximum radius of
  the non-rocky planet peak from, say, the dissolution of H/He into the planet's magma ocean
  at the high surface pressures expected for planets with $r_p\gtrsim 3$ R$_{\oplus}$ \citep{kite19}.}
...
The resulting locations of each radius peak and the
valley are depicted in \autoref{fig:rpvMs} as a function of
stellar mass. The feature locations and uncertainties are also given explicitly in \autoref{tab:rpvMs}.
The depicted $M_s$ values are represented by the median stellar mass within each bin with
uncertainties derived from the $16^{\text{th}}$ and $84^{\text{th}}$ percentiles. In computing the
feature locations we assume that the bimodal $r_p$ distribution is present in all stellar mass bins
aside from the lowest mass bin wherein the non-rocky peak becomes insignificant
(see \autoref{fig:rphistcomp}). In the lowest stellar mass bin, we
...
These feature radii are then compared to those measured by \cite{fulton18} around Sun-like stars with
...
\begin{figure*}
  \centering
  \includegraphics[width=.97\hsize]{figures/rpvMsFULL_KepK2_bkgd.png}%
  \hspace{-.97\hsize}%
  \begin{ocg}{fig:off4M}{fig:off4M}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on4M}{fig:on4M}{1}%
  \includegraphics[width=.97\hsize]{figures/rpvMsFULL_KepK2_models.png}%
  \end{ocg}
  \hspace{-.97\hsize}%
  \begin{ocg}{fig:off1M}{fig:off1M}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on1M}{fig:on1M}{1}%
  \includegraphics[width=.97\hsize]{figures/rpvMsFULL_KepK2_FP18.png}%
  \end{ocg}
  \hspace{-.97\hsize}%
  \begin{ocg}{fig:off3M}{fig:off3M}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on3M}{fig:on3M}{1}%
  \includegraphics[width=.97\hsize]{figures/rpvMsFULL_KepK2_bin.png}%
  \end{ocg}
  \hspace{-.97\hsize}%
  \begin{ocg}{fig:off2M}{fig:off2M}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on2M}{fig:on2M}{1}%
  \includegraphics[width=.97\hsize]{figures/rpvMsFULL_KepK2_Ms.png}%
  \end{ocg}
  \hspace{-.97\hsize}%
  \caption{(\emph{Interactive figure}) Evolution of the radius valley features with stellar mass. \emph{Solid markers}:
    ...
    retrieved from \cite{fulton18} \ToggleLayer{fig:on1M,fig:off1M}{\protect\cdbox{(\emph{open circles})}}.
    Feature radii from our full sample with a median value of
    $M_s = 0.651^{+0.058}_{-0.096}$ M$_{\odot}$ are depicted as
    \ToggleLayer{fig:on2M,fig:off2M}{\protect\cdbox{\emph{filled circles}}}.
    \ToggleLayer{fig:on3M,fig:off3M}{\protect\cdbox{\emph{Filled squares}}}
...
\autoref{fig:rpvMs} reveals that
the location of each feature, measured from our full stellar sample, continues the trend of monotonically
...
are $\text{d}r_{p,\text{rock}} / \text{d}M_s = 0.40$ and $\text{d}r_{p,\text{gas}} / \text{d}M_s = 0.97$
...
Models of the formation of the radius valley based on photoevaporation \citep{wu19}, gas-poor formation \citep{lopez18},
...
relation (MLR) $L_s \propto M_s^{\alpha}$. In \autoref{fig:rpvMs}, we consider predictions assuming either
a constant MLR with $\alpha=5$ \citep{gupta19b} or the empirically-derived piecewise MLR from
\cite{eker18}. All models predict a decreasing radius valley with decreasing stellar mass, but the models
differ in the predicted rates of this decrease. 
...
with decreasing stellar mass such that measurements of the valley's location in decreasing $M_s$ bins may be used to
...
$M_s$ bins prevent significant inferences with regards to the relative strength of the competing physical processes.
...
$P_{\text{valley,Kep}}=8.56\times 10^{-3}$, in order to scale the uncertainty on $f_{\text{valley}}$---and hence on the radius
valley location---as an increasing number of mid-to-late M dwarfs are observed by \tess{} with 2-minute cadence.

The resulting improvement in the measurement precision of the radius valley with observations of additional mid-to-late
M dwarfs is shown in \autoref{fig:improve}. The \tess{} curve therein
reveals how precisely the location of the radius valley can be measured given \tess{'s} approximate 
...
We define the target measurement precision as that which is required to distinguish between predictions from photoevaporation and
%core-powered mass loss (assuming an empirical mass-luminosity relation)
gas-poor formation at $3\sigma$ around low mass stars with a median
...
of precision would be required to distinguish between photoevaporation and core-powered mass loss, assuming the empirical
mass-luminosity relation from \cite{eker18}.
The approximate \tess{} detection sensitivity implies that \tess{} will be required to observe $\sim 85,000$ mid-to-late M dwarfs
to distinguish between model predictions of photoevaporation and gas-poor formation at $3\sigma$.
...
could achieve a radius valley measurement uncertainty of $\sim 0.17$ R$_{\oplus}$ by the end of its prime mission.
These observations will be useful for constraining radius valley formation models as predictions from photoevaporation and
gas-poor formation---or core-powered mass loss---could be distinguished at $\sim 2.1\sigma$ with this level of precision.
...
of the radius valley could have been precisely measured to 0.12 R$_{oplus}$ with $\sim 1200$ observed stars.

The primary goal of the up-coming ESA \plato{} mission \citep{rauer14}
is to detect and characterize transiting habitable zone planets around bright FGK stars, although 
the \plato{} stellar input catalog has yet to be defined.
Despite this, according to the mission's Definition Study
...
is very rough and not binding to the final definition of the PIC, we combine this expectation with knowledge of M dwarfs
in the solar neighborhood to estimate the number of mid-to-late M dwarfs in the P4 sample. We do so by retrieving
the M dwarf sample within 25 pc from \cite{winters19}. Noting that this volume-limited sample is expected to be 33\%
complete (J. Winters private communication), we identify $\sim 2829$ M dwarfs within 25 pc and with $V<16$.
We then scale up the M dwarf population beyond 25 pc until 5000 stars with $V<16$ are included assuming a homogeneous M
dwarf population beyond 25 pc. Of those bright M dwarfs representing probable targets within the P4 sample, 3358 have
masses $<0.4$ M$_{\odot}$. With this number of mid-to-late M dwarfs targeted by \plato{,} we expect the radius valley
uncertainty to reach $\sim 0.14$ R$_{\oplus}$ which would enable models of photoevaporation is be distinguished from
models of core-powered mass low or gas-poor formation at $\sim 2.6\sigma$.
...
$1.9$ R$_{\oplus}$ for $M_s \sim 1.2$ M$_{\odot}$ and $1.7$ R$_{\oplus}$ for $M_s \sim 0.85$ M$_{\odot}$
\citep{fulton18}.
...
radius valley slope measured in this work---consistent with models of gas-poor formation---to the slope measured
around Sun-like stars from the CKS sample \citep{martinez19}. In \autoref{fig:transition}, the latter is scaled
from its median stellar mass of 1.01 M$_{\odot}$ to the median stellar mass of our sample (0.65 M$_{\odot}$) using the
$M_s$-dependent scaling relation of the radius valley under photoevaporation
\citep[$r_p \propto (M_s/\text{M}_{\odot})^{1/4}$;][]{wu19}.
...
\begin{align}
  r_{p,\text{valley,this work}} &= 0.11\cdot \log_{10}(P)+1.52 \text{ and} \\
...
\noindent These subsets of the $P-r_p$ space define the regions of interest for resolving the
  ...
  atmospheric mass loss predicts that planets in the set defined by
  ...
  information is available to resolve a possibly sharp transition. \textbf{At the time of writing, \tess{}
    has identified eleven TOIs\footnote{\tess{} Object-of-Interest.}
    around stars with \teff{} $<4700$ K that lie within the region of interest defined by Eq.~\ref{eq:transition}
    with $P<23.52$ days. Ten of these TOIs have
    ...
    836.02, 873.01, 1075.01, and 1201.01.}.

\begin{figure}
  \centering
  \includegraphics[width=\hsize]{figures/fmapregion_bkgd_Msgt0d0_xbin39_ybin26.png}%
  \hspace{-\hsize}%
  \begin{ocg}{fig:off4pr}{fig:off4pr}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on4pr}{fig:on4pr}{1}%
  \includegraphics[width=\hsize]{figures/fmapregion_slope_Msgt0d0_xbin39_ybin26.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \begin{ocg}{fig:off1pr}{fig:off1pr}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on1pr}{fig:on1pr}{1}%
  \includegraphics[width=\hsize]{figures/fmapregion_planets_Msgt0d0_xbin39_ybin26.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \begin{ocg}{fig:off3pr}{fig:off3pr}{0}%
  \end{ocg}%
  \begin{ocg}{fig:on3pr}{fig:on3pr}{1}%
  \includegraphics[width=\hsize]{figures/fmapregion_tois_Msgt0d0_xbin39_ybin26.png}%
  \end{ocg}
  \hspace{-\hsize}%
  \caption{(\emph{Interactive figure}) Regions of interest for resolving the transition between rocky and non-rocky planets
    ...
    our sample. The \ToggleLayer{fig:on4pr,fig:off4pr}{\protect\cdbox{shaded regions}} bounded by
    these curves represent the subset of the $P-r_p$ parameter space of interest for resolving
    the transition around low mass stars with precise bulk density measurements. The eleven
    \ToggleLayer{fig:on3pr,fig:off3pr}{\protect\cdbox{TOIs}}
    that fall within this region are marked by an `x'. Also overplotted are
    \ToggleLayer{fig:on1pr,fig:off1pr}{\protect\cdbox{planets}}
    with $\geq 3\sigma$ bulk density measurements from the literature that are classified as having either
    a rocky (\emph{circles}), a gaseous (\emph{triangles}), or an intermediate (\emph{squares}) bulk
    composition. Marker colors are indicative of the MAP planet bulk densities.}
...
rather than to two distinct populations of rocky and non-rocky planets. However, if the radius valley is instead
sculpted by atmospheric post processing---as the radius valley around Sun-like stars appears to be
...
This seems plausible in the photoevaporation scenario as the atmospheric mass loss rate is proportional to
...
evolves from a positive slope to a negative slope from Sun-like to low mass stars. Unfortunately,
...
%Our occurrence rate calculations were based on
%the sets of confirmed planets from each mission along with their unique transit detection sensitivities.
%Notably,
%when \ktwo{} planet candidates are included and statistically corrected for contamination by astrophysical
%false positives, the cumulative occurrence rates of small close-in planets around low mass stars
%from \kepler{} and \ktwo{} are found to be consistent.
...
\item The radius valley around low mass stars exhibits a negative slope with insolation
  ...
  supports models of gas-poor terrestrial planet formation around low mass stars,
  without invoking atmospheric escape from photoevaporation,
...
\item We advocate that transiting planets around stars with \teff{} $<4700$ K, and whose orbital periods and radii are
  situated between model predictions of the location of the rocky/non-rocky transition (see \autoref{fig:transition}),
