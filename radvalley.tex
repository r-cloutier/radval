...
%for cases of $i$ representing the
%orbital period and insolation.
...
searches for transiting small close-in planets and secondly, by quantifying the detection sensitivity in those searches. 
...
by \tess{} to enable robust conclusions regarding the nature of the radius valley formation. These calculations
...
Among these stars the SPOC has reported three objects of interest close to the terrestrial to gaseous planet transition
...
as an increasing number of low mass stars are observed with 2 minute cadence with \tess{.}

The resulting improvement in the measurement precision of the radius valley with observations of additional low mass
stars is shown in Fig.~\ref{fig:improve}.
...
cadence is increased. Note that the improvement allotted by \tess{} should onluy be interpretted as an
approximation given that the detection sensitivity has not been adequately characterized. In our calculations the
detection sensitivity is estimated as a constant value as described in the preceding paragraph. 
...
the required precision corresponds to a radius vally uncertainty of $\sim 0.12$ R$_{\oplus}$. A very similar level
of precision would be required to distinguish between photoevaporation and gas-poor formation as well.
With the approximate \tess{} detection sensitivity, \tess{} will be required to observe $\sim 85,000$ mid-to-late M dwarfs
...
sensitivity is characterized by then and is roughly consistently with the approximate value assumed here then \tess{}
could achieve a radius valley uncertainty of $\sim 0.17$ R$_{\oplus}$. This would still be useful for constraining radius
valley formation models as predictions from photoevaporation and
core-powered mass low, or gas-poor formation, could be distingiushed at $\sim 2.1\sigma$ with this level of precision.
...
\begin{figure*}
  ...
  \caption{Expected improvement in the measurement precision of the radius valley location with the number of observed
    ...
    values for the \tess{} (\emph{yellow}) and \emph{PLATO} (\emph{orange}) missions. The curve for a hypothetical continuation
    of the primary \kepler{} mission is depicted in \emph{red}. The target precision of 0.12 R$_{\oplus}$ (\emph{shaded region})
    would enable models of photoevaporation to be distinguished from core-powered mass loss and gas-poor formation at $3\sigma$
    at $M_s=0.35$ M$_{\odot}$. Measurement precision curves are compared to samples of mid-to-late M dwarfs from the \tess{}
...
Also included in Fig.~\ref{fig:improve} are curves for a hypothetical \kepler{} mission and the up-coming \emph{PLATO}
mission. The hypothetical \kepler{} curve reveals that had \kepler{} been able to continue its prime mission and have
