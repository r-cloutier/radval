...
\begin{abstract}
  ...
  The location of the radius valley features are also shown to shift to smaller sizes with
...
\autoref{fig:fmapF} reveals that all planets that are inconsistent with having bulk terrestrial
...
frequency of terrestrial to gaseous planets in each stellar mass bin are reported in \autoref{tab:rel}
...
The values in \autoref{tab:rel} indicate the significant increase in the relative occurrence of terrestrial
...
due to the limited detection sensitivity to sub-Earth-sized planets. Typical core sizes may also be stellar mass
dependent as a core size for a fixed mass is composition dependent and the mass of heavy elements per star
...
between $1.4-1.6$ R$_{\oplus}$\footnote{\tess{} Objects-of-Interest, TOIs: 175.01, 406.01, and 667.01.}. 
...
predominantly terrestrial or gaseous planets. \autoref{fig:transition} reveals the region of interest for resolving
the terrestrial-to-gaseous transition in the $P-r_p$ space by comparing the radius valley slope measured in this work around
low mass stars to the slope measured around Sun-like stars from the CKS sample \citep{martinez19}. The latter is scaled
...
\textbf{The measured transitions around low mass and Sun-like stars
...
\noindent \textbf{These subsets of the $P-r_p$ space define the regions of interest for resolving the
  terrestrial-to-gaseous transition around low mass stars. For example at $P\lesssim 23.52$ days, thermally driven
  ...
  predominantly gaseous. These predictions can be robustly tested by targeting planets within the
  ...
  information
  is available to resolve a possibly sharp transition. The \tess{} SPOC has already identified three TOIs of interest
  around stars with \teff{} $<4700$ K that should be targeted by RV follow-up
  campaigns.\footnote{TOIs: 654.01, 732.02, and 1184.01.} }

\begin{figure}
  \centering
  \includegraphics[width=0.99\hsize]{figures/fmapregion_Msgt0d0_xbin39_ybin26.png}
  \caption{Regions of interest for resolving the transition between terrestrial and gaseous planets
    around low mass stars. The colormap depicts the occurrence rate of small close-in planets from
    \autoref{fig:fmap}. The terrestrial-to-gaseous transition measured in this work is highlighted
    ...
    the transition around low mass stars from precise bulk density measurements. The three TOIs from the
    SPOC that fall within this region are marked by an `x'. Also overplotted are planets
    with $\geq 3\sigma$ bulk density measurements from the literature that are classified as having either
    a terrestrial (\emph{circles}), a gaseous (\emph{triangles}), or an ambiguous (\emph{squares}) bulk
    composition.}
...
\item The radius valley around low mass stars exhibits a negative slope with orbital separation
  ($\text{d}r_p / \text{d}\log{F} = -0.123\pm 0.054$) unlike 
  around Sun-like stars whose measured slope is positive. These observations
  support models of gas-poor terrestrial planet formation without invoking atmospheric escape from photoevaporation,
...
\item The occurrence rate-weighted central radius of each planet peak and the radius valley shift to smaller
  sizes with decreasing stellar mass. The slope of the gaseous planet peak's radius with stellar mass is shown to be
  twice that of the terrestrial peak radius indicating that the planet population in each stellar mass bin
...
\item We advocate that transiting planets around stars with \teff{} $<4700$ K, and whose orbital periods and radii are
  situated between model predictions of the location of the terrestrial-to-gaseous transition (see \autoref{fig:transition}),
  should be targeted by RV campaigns to resolve location and slope of this transition using precise bulk density measurements. 
\end{itemize}
